-- Athlete System: NEW badge (updated_at vs seen_at) + helper RPCs for bump / mark seen.
...
COMMENT ON COLUMN public.profiles.athlete_board_updated_at IS
  'Last admin update to athlete status board (level, titles, growth). Drives member NEW badge.';
COMMENT ON COLUMN public.profiles.athlete_board_seen_at IS
  'When member last opened 나의 상태. NEW when updated_at > seen_at.';

CREATE OR REPLACE FUNCTION public.bump_athlete_board_updated(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET athlete_board_updated_at = now()
  WHERE id = p_user_id;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.bump_athlete_board_updated(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.mark_athlete_board_seen() TO authenticated;

-- Bump on EXP / level changes (admin_apply_member_stat_exp)
...
BEGIN
  ...
  PERFORM public.bump_athlete_board_updated(p_target_user);
