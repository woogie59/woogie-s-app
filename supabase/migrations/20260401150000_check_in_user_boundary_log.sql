-- check_in_user: no_sessions 실패 직전 경계 수치 로그(서버) + v_scheduled(오늘 KST~ 예약, 취소 제외)
-- 클라이언트와 동일: 잔여 = total - completed(출석); 예약 건수는 소모가 아님(로그·분석용).
...
COMMENT ON FUNCTION public.check_in_user(UUID) IS
  'QR 출석. 잔여: sum(session_batches) - 완료 출석. 실패 시 WARNING으로 경계 수치 로그.';
